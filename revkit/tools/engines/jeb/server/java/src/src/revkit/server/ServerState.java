...
public class ServerState {
    ...
    // Lazy HTTP start: server accepts requests before project is fully loaded
    private volatile boolean serverReady = false;

    // Cached class index (Optimization #22: avoid repeated dex.getClasses())
    private List<IDexClass> cachedClasses;
    private Map<String, IDexClass> classIndex;

    // Cached string list (Optimization #8: avoid repeated dex.getStrings())
    private List<String> cachedStrings;
    private int totalStringCount;

    // Cached original signatures (Optimization: avoid repeated getSignature() JEB API calls)
    private IdentityHashMap<Object, String> originalSigCache;
    ...
    public boolean isServerReady() { return serverReady; }
    public List<IDexClass> getCachedClasses() { return cachedClasses; }
    public IDexClass getClassBySig(String sig) {
        return classIndex != null ? classIndex.get(sig) : null;
    }
    public List<String> getCachedStrings() { return cachedStrings; }
    public int getTotalStringCount() { return totalStringCount; }
    /** Get cached original signature for a dex item, or empty string if not cached. */
    public String getOriginalSig(Object item) {
        if (originalSigCache == null) return "";
        String cached = originalSigCache.get(item);
    ...
    public void setServerReady(boolean v) { this.serverReady = v; }
    ...
    /**
     * Build class index from all dex units (Optimization #22).
     * Call once after dexUnits are set. Caches all classes and builds
     * sig-to-class lookup maps. Also sets totalClassCount (#10).
     *
     * Cache keys always use getSignature(false) (immutable original name) (#23).
     * The current-name form getSignature(true) is also indexed for convenience.
     */
    public void buildClassIndex() {
        List<IDexClass> all = new ArrayList<>();
        Map<String, IDexClass> idx = new HashMap<>();
        IdentityHashMap<Object, String> sigCache = new IdentityHashMap<>();
        if (dexUnits != null) {
            for (IDexUnit dex : dexUnits) {
                try {
                    for (Object cls : dex.getClasses()) {
                        IDexClass c = (IDexClass) cls;
                        all.add(c);
                        String origSig = c.getSignature(false);
                        idx.put(origSig, c);                // immutable original sig
                        idx.put(c.getSignature(true), c);   // current (possibly renamed) sig
                        sigCache.put(c, origSig);            // reverse lookup cache
                    }
                } catch (Exception e) { /* skip problematic dex */ }
            }
        }
        this.cachedClasses = Collections.unmodifiableList(all);
        this.classIndex = Collections.unmodifiableMap(idx);
        this.originalSigCache = sigCache;
        this.totalClassCount = all.size();
    }

    /**
     * Build string cache from all dex units (Optimization #8).
     * Call once after dexUnits are set. Caches all string values to avoid
     * repeated dex.getStrings() iteration on every strings query.
     */
    public void buildStringCache() {
        ...
        if (dexUnits != null) {
            for (IDexUnit dex : dexUnits) {
                try {
                    for (Object s : dex.getStrings()) {
                        String val = ((IDexString) s).getValue();
                        if (val != null) {
                            strings.add(val);
                        }
                    }
        ...
        this.cachedStrings = Collections.unmodifiableList(strings);
        this.totalStringCount = strings.size();
    }

    /** Get total class count (cached). Uses buildClassIndex() result if available (#10). */
    public int getTotalClassCount() {
        if (totalClassCount < 0) {
            int count = 0;
            if (cachedClasses != null) {
                count = cachedClasses.size();
            } else if (dexUnits != null) {
                for (IDexUnit dex : dexUnits) {
    ...
    /** Get total method count (cached). Uses cachedClasses if available. */
    public int getTotalMethodCount() {
        if (totalMethodCount < 0) {
            int count = 0;
            if (cachedClasses != null) {
                for (IDexClass cls : cachedClasses) {
                    try { count += cls.getMethods().size(); }
                    catch (Exception e) { /* skip */ }
                }
            } else if (dexUnits != null) {
                for (IDexUnit dex : dexUnits) {
