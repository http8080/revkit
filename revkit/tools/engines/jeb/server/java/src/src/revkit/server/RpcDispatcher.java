...
public class RpcDispatcher {
     ...
     */
    /** Methods available even before project is fully loaded. */
    private static final Set<String> ALWAYS_AVAILABLE = new HashSet<>(
        Arrays.asList("ping", "status", "methods", "stop", "save"));
    ...
    public JSONObject dispatch(String method, JSONObject params) {
        ...
        // Allow system methods anytime; block data methods until ready
        if (!state.isServerReady() && !ALWAYS_AVAILABLE.contains(resolved)) {
            throw new RpcException("SERVER_NOT_READY",
                "Server is still analyzing. Try again shortly.",
                "Wait for 'ready' state before calling data methods.");
        }
